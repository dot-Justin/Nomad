...
const { tmux, parseSessions, parseWindows, safeName } = require("./tmux");
...
class SshSession {
  ...
  _openConnection(opts) {
    return new Promise((resolve) => {
      ...
      conn.on("error", (err) => {
        ...
        if (!this.disposed && this.attachedSession) {
  ...
  async _attachSession(sessionName) {
    const name = safeName(sessionName) || "nomad";
    if (!this.conn) return;
    return new Promise((resolve) => {
      this.conn.shell(
        {
        ...
        (err, stream) => {
          ...
          stream.write(`${tmux.attachOrCreate(name)}\n`);
          resolve();
        }
      );
    });
  ...
  async killSession({ sessionName }) {
    ...
    if (this.attachedSession === safeName(sessionName)) {
      this.attachedSession = null;
      if (this.stream) {
        try {
          this.stream.end();
  ...
  async newWindow() {
    this.sendKeys("\x02c");
    setTimeout(() => this.listWindows(), 250);
  }

  async killWindow({ windowIndex }) {
    if (!this.conn || !this.attachedSession) return;
    if (typeof windowIndex === "number") {
      await runCommand(this.conn, tmux.killWindow(windowIndex));
    } else {
      this.sendKeys("\x02&");
      this.sendKeys("y\n");
    }
  ...
  async renameWindow({ index, name }) {
    if (!this.conn || index == null) return;
    await runCommand(this.conn, tmux.renameWindow(index, name));
  ...
  async scrollUp() {
    this.sendKeys("\x02[\x15");
  }

  async scrollDown() {
    this.sendKeys("\x02[\x04");
  }

  async scrollSearch({ query }) {
    if (!query) return;
    const safe = String(query).replace(/[\r\n\x1b]/g, "");
    this.sendKeys(`\x02[/${safe}\n`);
  }
  ...
  detach() {
    this.sendKeys("\x02d");
    this.attachedSession = null;
    setTimeout(() => this.dispose(true), 200);
  }
